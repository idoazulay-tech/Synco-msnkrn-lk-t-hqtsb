...
export async function storeUserMessage(
  userId: string,
  text: string,
  meta?: { type?: string; source?: string; pointId?: string; [key: string]: unknown }
): Promise<string> {
  // Use caller-supplied deterministic ID when provided (learning-event derived memories).
  // Fall back to a random UUID for all other non-learning memory writes — unchanged behavior.
  const { pointId: callerPointId, type, source, ...extraPayload } = meta ?? {};
  const pointId = callerPointId ?? uuid();
...
export async function searchMemory(
  ...
  const { vector, isFallback } = await generateEmbedding(query);
  ...
  try {
    ...
    // Qdrant search errors (version mismatch, fallback-embedding incompatibility,
    // transient connectivity) are non-fatal — return empty results gracefully.
    const msg = err instanceof Error ? err.message : String(err);
    console.warn(`[Memory] searchMemory failed for collection '${collectionName}' (fallback=${isFallback}): ${msg}`);
    return [];
  }
}
