...
router.get('/retrieve', async (req, res) => {
  try {
    ...
    const totalResults = result.wikiEntries.length + result.signals.length + result.graphNodes.length;
    ...
    const response: Record<string, unknown> = {
      ...
      wikiEntries: result.wikiEntries.map(w => ({
        ...
        sourceSignalIds: (w as any).sourceSignalIds ?? [],
        confidence:      w.confidence,
        sensitivityLevel: (w as any).sensitivityLevel,
      ...
      signals: result.signals.map(s => ({
        ...
        summary:         s.summary,
        confidence:      s.confidence,
        shouldCreateTask: (s as any).shouldCreateTask,
        shouldUpdateWiki: (s as any).shouldUpdateWiki,
        shouldUpdateGraph: (s as any).shouldUpdateGraph,
        sensitivityLevel: (s as any).sensitivityLevel,
      ...
      graphNodes: result.graphNodes.map(n => ({
        ...
        sensitivityLevel: (n as any).sensitivityLevel,
    ...
    if (devMode === 'true') {
      response.diagnostics = [
        `retrieve: userId=${userId.trim()}, query="${query.trim()}"`,
        ...result.diagnostics,
      ];
    }
